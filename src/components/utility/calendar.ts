...
import { TimeSteps, dimensionItemsType } from "../types";
...
export function iterateTimes( // The start time in the format "YYYY-MM-DDTHH:mm:ss.SSSZ".
  start: number,
  // The end time in the format "YYYY-MM-DDTHH:mm:ss.SSSZ".
  end: number,
  // The unit of time, such as "seconds" or "minutes".
  unit: keyof typeof defaultTimeSteps,
  // An object that maps units to time steps. For example, {"seconds": 10} would mean that the function should iterate every 10 seconds.
  timeSteps: typeof defaultTimeSteps,
  // A function that takes in two strings, representing the current time and the next time, and returns nothing.
  ...
  Object.keys(timeDividers).some((typedUnit) => {
    ...
    if (typedUnit in timeDividers) {
      ...
      const cellWidthToUse =
        timeSteps[unit] && timeSteps[unit] > 1
          ? 3 * minCellWidth
          : minCellWidth;
...
export function calculateInteractionNewTimes({
  ...
  itemTimeEnd: number;
  dragTime?: number | undefined | null;
  isDragging: boolean;
  isResizing: boolean;
...
export function getGroupOrders(
  groups: any[],
  keys: { groupIdKey: string },
): Record<string, { index: number; group: any }> {
  const { groupIdKey } = keys;

  let groupOrders: Record<string, { index: number; group: any }> = {};
...
export function getGroupedItems(
  items: any[],
  groupOrders: Record<string, { index: number; group: any }>,
): Record<number, { index: number; group: any; items: any[] }> {
  var groupedItems: Record<
    number,
    { index: number; group: any; items: any[] }
  > = {};
...
export function collision(
  ...
  return (
    (a.collisionLeft ?? 0) + collisionPadding <
      b.collisionLeft + b.collisionWidth &&
    (a.collisionLeft ?? 0) + (a.collisionWidth ?? 0) - collisionPadding >
      b.collisionLeft &&
...
export function groupNoStack(
  ...
  groupHeight: number,
  groupTop: number,
): { groupHeight: number; verticalMargin: number; itemTop: number } {
  let verticalMargin = (lineHeight - item.dimensions.height) / 2;
...
export function stackGroup(
  ...
  lineHeight: number,
  groupTop: number,
): { groupHeight: number; verticalMargin: number } {
  var groupHeight = 0;
  var verticalMargin = 0;
...
export function stackTimelineItems(
  ...
  let dimensionItems: any = visibleItemsWithInteraction
    .map((item) =>
      getItemDimensions({
        item,
        keys,
...
export function getItemWithInteractions({
  ...
  const newItem = {
    ...
    [keys.itemGroupKey]: isDragging
      ? _get(newGroupOrder ? groups?.[newGroupOrder] : "", keys.groupIdKey)
...
export function getCanvasBoundariesFromVisibleTime(
  ...
  const canvasTimeStart = visibleTimeStart - (zoom * (1 - 1)) / 2;
  const canvasTimeEnd = canvasTimeStart + zoom;
...
export function calculateScrollCanvas(
  ...
  if (!canKeepCanvas || forceUpdateDimensions) {
    ...
    const canvasWidth = mergedState.width;
